...
- 設定と演出計算の1拍は四分音符1つ分です。拍枠と拍数カウンターだけは、先頭拍子の分母音符を1拍として使用します。
- 途中の拍子変更は小節枠だけへ反映します。拍枠と拍数カウンターは変更後の拍子へ追従しません。
- 曲途中の拍子の分母変更は対応対象外です。
...
- レベルメータは20段固定で、Zone境界は感度から線形算出します。Peak Hold、短音用の固定Release、Note Off同期Releaseは設定画面から変更できません。
- ノート基本発光を高くすると、近距離ノートの色が白寄りに飽和する場合があります。遠方対策には基本発光ではなく、遠方視認性を使用します。
- 遠方視認性を`1`にすると、ノートとレベルメータからFogによる奥行き感が失われます。
- Note Onエフェクト4種とロングトーン粒子は常にFog無効で、遠方視認性の影響を受けません。
- Note Onエフェクト4種は距離に応じて見かけのサイズが小さくなります。
- ロングトーンFadeはノート全体のOpacityを下げる方式で、バー自体の切り詰めは行いません。

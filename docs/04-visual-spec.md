...
- Fade設定と演出計算の1拍は、拍子にかかわらず四分音符1つ分とする。
...
- Time Signatureを区間ごとに適用し、PPQから小節境界を計算する。
- 拍子変更イベントのtickを変更後の拍子の小節頭として扱う。

...
保存Statusは常時表示しません。保存成功時は「保存しました」を短時間表示し、`localStorage`への保存失敗時は「保存に失敗しました」を表示し続けます。
...
`showMeasureCounter`はBPMと拍数カウンター全体の表示フラグです。内部名は拍数カウンターだけを示すように見えますが、BPM表示も同時に制御します。
...
- レベルメータ設定は生成済みInstanceのMaterial、色、配置、Scaleへ即時反映する。
...
- `localStorage`の保存値から現行の設定キーだけを読み込む。
- 廃止済みまたは未知のキーは無視する。
- 保存値のキー名変換は行わない。
- 現行設定へ追加されたキーは初期値で補完する。

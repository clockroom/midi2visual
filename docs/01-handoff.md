...
| `src/shared/track-order.ts` | TrackCollection内部で使用するTrack順序計算 |
...
- Tempo変更は対応済みです。
- 設定と演出計算の1拍は四分音符1つ分、つまり`PPQ ticks`です。
- 拍枠と拍数カウンターだけは、先頭拍子の分母音符を`musicalBeat`として使用します。
...
- 短いノートでは固定`300ms` Envelopeを使用し、再発音時は新しいVelocityと現在Levelの大きい方を使用する

...
遠方のノートがFogで暗くなる場合は、「遠方ノート視認性」を上げるとノートだけFogの影響を弱められます。`0`は通常のFog、`1`はノートへのFogを無効化した状態です。「ノート基本発光」は距離対策ではなく、ノート自体の最低発光量の調整に使用します。
...
ロングトーン粒子化を有効にすると、実際のFade時間の50%地点でバーとGlowを粒子へ置き換え、残りのFade時間でTrack色の粒子が拡散しながら消えます。粒子化範囲はノート長に対する割合で指定し、初期値は`60%`です。1ノートあたりの最大粒子数と、距離減衰しないScreen Space基準の粒子サイズも調整できます。

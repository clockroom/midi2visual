...
遠方のノートやレベルメータがFogで暗くなる場合は、「空間とカメラ」の「遠方視認性」を上げると両方のFogの影響を弱められます。`0`は通常のFog、`1`は対象へのFogを無効化した状態です。「ノート基本発光」は距離対策ではなく、ノート自体の最低発光量の調整に使用します。Note Onエフェクト4種とロングトーン粒子は常にFog無効ですが、Note Onエフェクト4種の距離によるサイズ変化は維持されます。

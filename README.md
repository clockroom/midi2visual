...
使用頻度の高いファイル、ロングトーンエフェクト、カスタムエフェクト、レベルメータは常時表示されます。HTMLと狭幅表示ではこの順に並び、PCではファイルとロングトーンエフェクト、レベルメータとカスタムエフェクトをそれぞれ横並びにします。

ノート、ガイド、エフェクト、カメラ、再生は、下部の「詳細設定」からまとめて開閉できます。PCではノートが左側の2行を占め、その右へガイドとエフェクトを縦に配置し、最下段へカメラと再生を並べます。詳細設定はページ読み込み時に閉じた状態へ戻ります。
...
遠方のノートやレベルメータがFogで暗くなる場合は、「カメラ」の「遠方視認性」を上げると両方のFogの影響を弱められます。`0`は通常のFog、`1`は対象へのFogを無効化した状態です。「ノート基本発光」は距離対策ではなく、ノート自体の最低発光量の調整に使用します。Note Onエフェクト4種とロングトーン粒子は常にFog無効ですが、Note Onエフェクト4種の距離によるサイズ変化は維持されます。

ロングトーンFadeを有効にすると、初期設定ではNote Onから2拍でFadeを開始し、そこから6拍かけて完全に非表示になります。Fade完了より前にNote Offへ到達する場合はFade時間を短縮し、Note Off時点で完全に消えます。Fade対象になったノートへ通常のNote Off残光は適用しません。

ロングトーン粒子化を有効にすると、実際のFade区間内で指定したタイミングにバーとGlowを粒子へ置き換え、残りのFade時間でTrack色の粒子が拡散しながら消えます。初期タイミングは`50%`です。「粒子化前に発光 (秒)」が`0`より大きい場合は粒子化直前に強発光を加え、`0`では発光しません。粒子化範囲はノート長に対する割合で指定し、初期値は`60%`です。1ノートあたりの最大粒子数と、距離減衰しないScreen Space基準の粒子サイズも調整できます。

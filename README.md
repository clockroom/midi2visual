...
使用頻度の高いファイル、レベルメータ、ノート、ロングトーン、組み込みエフェクト、カスタムエフェクトは常時表示されます。低頻度の「再生と時間」「ガイド」「空間・カメラ・背景」は、下部の「詳細設定」からまとめて開閉できます。詳細設定はページ読み込み時に閉じた状態へ戻ります。
...
遠方のノートやレベルメータがFogで暗くなる場合は、「空間・カメラ・背景」の「遠方視認性」を上げると両方のFogの影響を弱められます。`0`は通常のFog、`1`は対象へのFogを無効化した状態です。「ノート基本発光」は距離対策ではなく、ノート自体の最低発光量の調整に使用します。Note Onエフェクト4種とロングトーン粒子は常にFog無効ですが、Note Onエフェクト4種の距離によるサイズ変化は維持されます。
